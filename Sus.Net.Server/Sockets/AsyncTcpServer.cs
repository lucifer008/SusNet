...
namespace Sus.Net.Server.Sockets
{
    ...
    {
        private ILog log = LogManager.GetLogger(typeof(AsyncTcpServer));
        ...
        public void Send(TcpClient tcpClient,byte[] datagram)
        {
            ...
            try
            {
                ...
                if ( stream.CanWrite )
                {
                    ...
                    log.Info(string.Format("    Socket:已经发送消息到终端[{0}]", tcpClient.Client.RemoteEndPoint.ToString()));
                }
            }
            catch ( ObjectDisposedException ex )
            {
