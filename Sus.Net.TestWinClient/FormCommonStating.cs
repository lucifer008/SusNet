...
namespace Sus.Net.TestWinServer
{
    public partial class FormCommonStating : Form
    {
        ...
        SusTCPClient susTCPClient;
        public FormCommonStating(byte[] bb,List<Hanger> _hangerList, SusTCPClient tcpClient,String statingNo, FormClientNew fClient ) : this()
        {
            ...
            susTCPClient = tcpClient;
            susTCPClient.SendMessageToAll(data);

            var modelCompareFlow = new MessageModel()
            {
                lvMessage = susTCPClient.LvAllMessage,
                Message = string.Format("【比较工序指令发送成功！发送内容:{0}】", fData)
            };
            fClient.AddMessage(modelCompareFlow, null);
                ...
                e.RowIndex >= 0)
            {
                ...
                if (e.ColumnIndex == 1)
                {
                    try
                    {
                        ...
                        var list = dataGridView1.DataSource as List<Hanger>;
                        var hanger = list[e.RowIndex];
                        var rework = new FormRework(hanger, susTCPClient);
                        rework.Show();
                    }
                    catch (Exception ex)
                    {
